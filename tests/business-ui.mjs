...
const server = await createServer({configFile:false,server:{host:'127.0.0.1',port:0},plugins:[{
  ...
  configureServer(vite) { vite.middlewares.use((req,res,next)=>{
    if(['/', '/admin','/producto/parlante'].includes((req.url||'').split('?')[0])) {
...
try {
  ...
  for(const [path,width] of [['/admin',1366],['/admin',1440],['/admin',1920],['/',320],['/',375],['/',430],['/producto/parlante',375]]) {
    ...
    await wait(path==='/admin'?"document.querySelector('.business-goal')":path==='/'?"document.querySelector('.store-product-card')":"document.querySelector('.store-detail-info')")
  ...
  await wait("document.querySelector('.store-product-card')")
  await click('Agregar');await click('Agregar');await click('Agregar')
  await evaluate("document.querySelector('[aria-label=\"Abrir carrito\"]').click()")
  await wait("document.querySelector('.store-qty input')?.value === '3'")
  ...
  await wait("document.body.innerText.includes('Stock insuficiente')")
  assert.equal(await evaluate("document.querySelector('.store-qty input').value"),'2')
